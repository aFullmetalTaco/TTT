...
for c , d in pairs(weapons.GetList()) do
	...
	if table.HasValue( weapons, weapon ) or table.HasValue( hl2weps, weapon ) then
		ply:Give( weapon )
	else
	ULib.tsay( ply, weapon .. " is not installed on this server.", true )
	end
	return true
end
end
...
give:addParam{ type=ULib.cmds.StringArg, hint="weapon_", ULib.cmds.round }
give:defaultAccess( ULib.ACCESS_ADMIN )
give:help( "Gives the player the specified weapon." )
give:logString( "#1s gave #3s to #2s." )
