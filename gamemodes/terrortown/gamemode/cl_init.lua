...
--
local function makeDLogVGUI() --Makes the VGUI on F4

	local DermaPanel = vgui.Create( "DFrame" )
	DermaPanel:SetPos( 400, 100 )
	...
	local LogText = vgui.Create( "DTextEntry" )
	LogText:SetPos( 60,25 )
	LogText:SetTall( 1000 )
	LogText:SetWide( 50 )
	LogText:SetMultiline(true)
	DermaList:AddItem( LogText )
	...
	PropertySheet:AddSheet( "Log Menu", DermaList , "gui/silkicons/user", false, false, "DamageLogzzz" )
	PropertySheet:AddSheet( "Super Menu", SheetItemTwo, "gui/silkicons/group", false, false, "/" ) 
	
end
usermessage.Hook("makeDLogVGUI", makeDLogVGUI) --Hooks the usermessage to the function

local function liveData( data ) --Updates the damagelog

end
usermessage.Hook("liveData", liveData) --Hooks the usermessage

local function updateLoglist( data ) --updates the log list...

end
usermessage.Hook("updateLoglist", updateLoglist)